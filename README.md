...
---
> #### 배포 주소
➡️ Production 배포 URL: https://account.ozcoding.site/


## 서비스 소개
StudyHub는 온라인 강의 수강 중도 포기를 줄이기 위해
스터디 기반의 학습 구조를 제공하는 플랫폼으로,
학습자 간의 연결과 일정·진행 관리 기능을 통해
강의를 끝까지 완주할 수 있는 환경을 만듭니다.
...
- 회원가입
   - 기본 정보 입력 (이름, 닉네임, 생년월일, 성별) → 닉네임 중복 확인
   - 이메일 인증: 이메일 입력 → 인증코드 발송 → 인증번호 확인
   - SMS 인증: 휴대폰 번호 입력 → 인증번호 발송 → 인증번호 확인
   - 비밀번호 설정 → 모든 인증 완료 후 가입하기

- 로그인 / 로그아웃
   - 일반 로그인: 이메일 + 비밀번호 입력 → 토큰 발급
   - 소셜 로그인: 카카오 / 네이버 간편 로그인 지원

- 토큰 기반 인증 처리
###  3. 이메일 & 비밀번호 찾기
  - 이메일 찾기 
       - 이름 + 휴대폰 번호 입력 → SMS 인증번호 발송 → 인증 완료 시 가입된 이메일 확인
  - 비밀번호 찾기
      - 이메일 입력 → 이메일로 인증코드 발송 → 인증 완료 → 새 비밀번호 설정
...
https://github.com/user-attachments/assets/5940c8b1-b752-4f1b-b3f5-f80864793e3f
...
<table>
  <tbody>
    <tr>
      ...
      <td align="center">
        <a href="https://github.com/dabinkwon">
          <img src="https://avatars.githubusercontent.com/u/183258837?v=4" width="100" height="100"/><br/>
          권다빈
        </a>
      </td>
      <td align="center">
        <a href="https://github.com/hwank1">
          <img src="https://avatars.githubusercontent.com/u/223783213?v=4" width="100" height="100"/><br/>
          기정환
        </a>
      </td>
    </tr>
